...
class QuizApp {
    ...
    renderSubjectSelector() {
        ...
        subjects.forEach((subj, index) => {
            ...
            button.innerHTML = `
                ...
                <small>單選 ${singleCount} 題 / 多選 ${multiCount} 題</small>
            `;
    ...
    renderUnitSelector() {
        ...
        if (totalMulti > 0) {
            const allMultiButton = document.createElement('button');
            allMultiButton.className = 'unit-btn';
            allMultiButton.style.background = 'linear-gradient(135deg, #e74c3c 0%, #c0392b 100%)';
            allMultiButton.innerHTML = `
                <strong>全部單元綜合測驗</strong><br>
                <small>${totalMulti} 道題目</small>
            `;
            allMultiButton.addEventListener('click', () => this.startQuiz(-1, 'multi'));
            this.multiUnitButtons.appendChild(allMultiButton);
        }
    }
